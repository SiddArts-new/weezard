...
const applyParticleEffect = (
  ...
  function generateParticle() {
    ...
    const speedHorz = (options?.speedHorz || Math.random() * 3) * (Math.random() <= 0.5 ? -1 : 1);
    const speedUp = -(options?.speedUp || Math.random() * 8);
    const spinVal = Math.random() * 360;
    const spinSpeed = Math.random() * 15 * (Math.random() <= 0.5 ? -1 : 1);
    const top = mouseY + window.innerHeight * 0.1;
  ...
  function refreshParticles() {
    particles.forEach((p) => {
      ...
      p.speedUp = Math.max(-p.size * 0.3, p.speedUp - 0.2);
      p.spinVal = p.spinVal + p.spinSpeed;

      if (p.top <= -p.size || p.top >= window.innerHeight + p.size) {
